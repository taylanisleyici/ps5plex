...
(
  ...
  # Tell Plex to look, so changes appear without waiting for a scheduled scan.
  ...
  # timer. managed.json changing is the signal for picks; the subs directory's
  # mtime changes when a subtitle is saved. The interval is just a backstop for
  # torrents that only become playable later.
  last=""
  while true; do
    now=$(stat -c %Y /state/managed.json /state/subs 2>/dev/null | tr '\n' ' ')
    if [ "$now" != "$last" ]; then
